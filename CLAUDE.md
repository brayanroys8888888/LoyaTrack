...
`Locataire.statut` is a French-string state machine (`Nouveau / Payé / En retard / En discussion / En pénalité`) mapped to Dart enums in `lib/models/models.dart` (`statutFromString`/`_statutToBackend`) — change both sides together. **New tenants default to `Nouveau`** (the frontend create payload omits `statut`); the status is then **recomputed**, not set by hand: `locataires/gestion.py::recalculer_statut` → `Payé` when the current month is covered (period-based, via `paiements.services.montant_du_ce_mois`), `En retard` once the échéance passes unpaid (and after the billing start), else stays `Nouveau`; `En discussion`/`En pénalité` are never downgraded except when the month is settled. It runs after each payment, via the daily Celery task `locataires.tasks.recalculer_statuts` (beat, before `calculer_penalites`), and inside `forcer_automatisations`. `Nouveau` is an automatic state — not offered in the manual status selectors. **First-payment anchoring:** `appliquer_paiement` sets `date_debut_facturation` + `jour_echeance` from the first payment's date (if not already set); penalties never accrue for a month before that billing start. Tenants also carry caution, ID-document, and lifecycle fields; soft-delete via `is_deleted`, and end-of-lease via `archive` (archived tenants are excluded from lists/dashboard unless `?inclure_archives=1`). Lifecycle business logic (rent increase, caution movements, résiliation) lives in `locataires/gestion.py` and is exposed as `LocataireViewSet` actions (`augmenter_loyer`, `verser_caution`, `restituer_caution`, `resilier`).
